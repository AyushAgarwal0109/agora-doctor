...
export default function Video(props) {
  ...
  const [patJoined, setPatJoined] = useState(false);
  ...
  useEffect(() => {
    ...
    if (now > 599 && now <= 600) {
      ...
      if (
        window.confirm('Do you want to extend the time limit by 3 minutes?')
      ) {
        renewAccessToken()
  ...
  useEffect(() => {
    if (wasExtended) {
      toast.success('3 minutes extended successfully!', {
        duration: 5000,
        position: 'top-right',
      });
    }
  ...
  useEffect(() => {
    ...
    if (
      meetingInfo &&
      meetingInfo.patient &&
      !meetingInfo.patient.inCall &&
      patJoined
    ) {
      setPatJoined(false);
      toast.success('Patient disconnected from the channel', {
        duration: 3000,
        position: 'top-right',
      });
    } else if (
      ...
      setPatJoined(true);
      if (meetingInfo.patient.disconnections > 0) {
        toast.success('Patient reconnected to the channel', {
          duration: 3000,
          position: 'top-right',
        });
      } else {
        toast.success('Patient joined the channel', {
          duration: 3000,
          position: 'top-right',
        });
      }
